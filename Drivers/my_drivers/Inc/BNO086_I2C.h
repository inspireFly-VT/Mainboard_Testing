...
// For telescope use we need to be able to calibrate and change the orienta
...
#else
//Features that can be used. Play with the conditions above to activate
...
#define FRS_RECORDID_SERIAL_NUMBER
#define FRS_RECORDID_SYSTEM_ORIENTATION
// FRS write status values
#define FRS_WRITE_STATUS_RECEIVED
#define FRS_WRITE_STATUS_UNRECOGNIZED_FRS_TYPE 1
#define FRS_WRITE_STATUS_BUSY
#define FRS_WRITE_STATUS_WRITE_COMPLETED
#define FRS_WRITE_STATUS_READY
#define FRS_WRITE_STATUS_FAILED
#define FRS_WRITE_STATUS_NOT_READY
#define FRS_WRITE_STATUS_INVALID_LENGTH
#define FRS_WRITE_STATUS_RECORD_VALID
#define FRS_WRITE_STATUS_INVALID_RECORD
#define FRS_WRITE_STATUS_DEVICE_ERROR
#define FRS_WRITE_STATUS_READ_ONLY
// Status values
...
UpgradeApp, //1 – Upgrade Application Mode; upgrade the application ima
ValidateImage, //2 – Validate Image Mode; validate an application image
LauchApp, //3 – Launch Application; launch the application image in fla
BootInvalid
} BNO_BootMode_t;
typedef enum {
MOTION_INTENT_UNKNOWN = 0, // FME_MOBILE_MOTION_INTENT_UNKNOWN – this i
...
uint8_t sw_Maj; // report 0xF8 byte 2 SW Ve
uint8_t sw_Min; // report 0xF8 byte 3 SW Ve
uint32_t sw_PN; // report 0xF8 byte 4-7 SW
uint32_t sw_BN; // report 0xF8 byte 8-11 SW
uint16_t sw_VP; // report 0xF8 byte 12-13 S
} BNO_productID_t;
typedef struct __attribute__((packed)) {
uint8_t status;
uint16_t wordOffset;
...
typedef struct __attribute__((packed)) {
uint8_t Severity; // Severity – the severity of the error currently b
uint8_t SeqNo; // Error sequence number. A monotonically increment
...
#endif
typedef struct __attribute__((packed)) {
uint8_t Status; // Status (0 – success. Non Zero – Oper
uint8_t AccCalEnable; // Accel Cal Enable (1 – enabled, 0 – disab
uint8_t GyroCalEnable; // Gyro Cal Enable (1 – enabled, 0 – disabled)
uint8_t MagCalEnable; // Mag Cal Enable (1 – enabled, 0 – disable
uint8_t PlanCalEnable; // Planar Accel Cal Enable (1 – enabled, 0 – di
uint8_t OnTableEnable; // On Table Cal Enable (1 – enabled, 0 – disabl
...
#endif
#if defined(ACCELEROMETER) || defined(LINEAR_ACCELERATION
// Accelerometer
typedef struct __attribute__((packed)) {
float X;
float Y;
...
uint8_t sequence; // The sequence number increments once for each repor
uint8_t status; // bits 7-5: reserved, 4-2: exponent delay, 1-0: Accura
...
#endif
#ifdef ACCELEROMETER
// Calibrated Acceleration (m/s2). Acceleration of the device including gra
BNO_Accelerometer_t getaccelerometer(void);
#endif
#ifdef LINEAR_ACCELERATION
// Linear acceleration (m/s2). Acceleration of the device with gravity remo
...
#endif
#ifdef RAW_GYROSCOPE
// Raw uncalibrated gyroscope (ADC units). Data direct from the gyroscope,
...
#endif
#ifdef GYROSCOPE_UNCALIBRATED
// Uncalibrated gyroscope (rad/s). Angular velocity without bias compensati
BNO_GyroscopeUncalibrated_t getGyroscopeUncal(void);
#endif
#ifdef RAW_MAGNETOMETER
// Raw magnetic field measurement (in ADC units). Direct data from the magn
BNO_RawMagnetometer_t getRawMagnetometer(void);
#endif
#ifdef MAGNETIC_FIELD_CALIBRATED
// Magnetic field calibrated (in µTesla). The fully calibrated magnetic fie
...
#endif
#ifdef ROTATION_VECTOR
// THE ROTATION VECTOR PROVIDES an orientation output that is expressed as
// and gravity. It is produced by fusing the outputs of the accelerometer,
// vector is the most accurate orientation estimate available. The magnetom
...
// heading, while roll and pitch are referenced against gravity. It is prod
// accelerometer and the gyroscope (i.e. no magnetometer). The game rotatio
// magnetometer to correct the gyroscopes drift in yaw. This is a deliberat
// allow gaming applications to use a smoother representation of the orient
// instantaneous correction provided by a magnetic field update could provi
BNO_RotationVector_t getGameRotationVector(void);
#endif
#ifdef GEOMAGNETIC_ROTATION_VECTOR
// THE GEOMAGNETIC ROTATION VECTOR is an orientation output that is express
// north and gravity. It is produced by fusing the outputs of the accelerom
// specifically excluded in order to produce a rotation vector output using
// rotation vector of section 2.2.4. The consequences of removing the gyros
// • Less responsive output since the highly dynamic outputs of the gyrosco
// • More errors in the presence of varying magnetic fields
BNO_RotationVectorWAcc_t getGeoMagRotationVector(void);
#endif
#ifdef PRESSURE
// The pressures sensor reports atmospheric pressure. The units are hectopa
float getPressure(void);
#endif
#ifdef AMBIENT_LIGHT
// The ambient light sensor reports the measures the amount of light enteri
float getAmbientLight(void);
#endif
#ifdef HUMIDITY
// The humidity sensor reports relative humidity in the ambient air. The un
float getHumidity(void);
#endif
#ifdef PROXIMITY
// The proximity sensor reports distance from the device. The units are cen
...
#endif
#ifdef SIGNIFICANT_MOTION
// The significant motion detector sends a single report when it detects si
...
#endif
#ifdef STABILITY_DETECTOR
// The stability detector sends a report each time it detects entry in or e
...
#endif
#ifdef POCKET_DETECTOR
// The pocket detector sends a report each time it detects entry in or exit
...
#endif
#ifdef HEART_RATE_MONITOR
// The heart rate monitor measures the user’s heart rate reported in beats
uint16_t getHeartRateMonitor(void);
#endif
#ifdef ARVR_STABILIZED_RV
// AR/VR STABILIZED ROTATION VECTOR
// Estimates of the magnetic field and the roll/pitch of the device can cre
// vector produced. For applications (typically augmented or virtual realit
// disturbing, the output is adjusted to prevent these jumps in a manner th
// sensor system. This process is called AR/VR stabilization. An FRS (Flash
...
// While the magnetometer is removed from the calculation of the game rotat
// create a potential correction in the rotation vector produced (i.e. the
// (typically augmented or virtual reality applications) where a sudden jum
// to prevent these jumps in a manner that takes account of the velocity of
// AR/VR stabilization. An FRS (Flash Record System – see Figure 1-31) reco
BNO_RotationVector_t BNO_getArVrStabilizedGRV(void);
#endif
#ifdef GYRO_INTEGRATED_RV
// GYRO ROTATION VECTOR
// Head tracking systems within a virtual reality headset require low laten
// the BNO08X can provide a rotation vector at rates up to 1kHz. The gyro r
// orientation to the standard rotation vector. Compared to the standard ro
// an optimized processing path and correction style (correction is the adj
...
#endif
#ifdef DEAD_RECKONING_POSE
// The Dead Reckoning Pose report provides the linear position, angular pos
